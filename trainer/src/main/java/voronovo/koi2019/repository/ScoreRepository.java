...
public interface ScoreRepository extends PagingAndSortingRepository<Score, Long> {
    @Query(value = "select new Score(min(s.name), min(s.category), min(s.time), min(s.percent)) from Score s where s.category like :category group by lower(s.name) order by min(s.time) asc, min(s.name) desc",
